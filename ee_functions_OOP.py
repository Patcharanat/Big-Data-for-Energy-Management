...
class ee_functions:
    ...
    def make_data_sample(self, floor=4):
        """
        Create a sample dataset for developing ML model Using Floor4 all zone year 2018, 2019 as the sample
        """
    ...
    def choose_zone(self, zone: dict, selected=[]):
        """
    ...
    def form_dataframe(self, data_array, old_dataframe):
        """
    ...
    def set_date_as_index(self, dataframe):
        """
    ...
    def set_index_as_index(self, df):
        """
    ...
    def train_test_split_data(self, df_input, df_output, train_size=0.8):
        """
        ...
        X_train_copy = self.set_date_as_index(X_train)
        X_test_copy = self.set_date_as_index(X_test)
        y_train_copy = self.set_date_as_index(y_train)
        y_test_copy = self.set_date_as_index(y_test)
        
        return X_train_copy, X_test_copy, y_train_copy, y_test_copy

    def reduce_resolution(self, df, interval=15):
        """
    ...
    def process_data(self, df):
        ...
        """
        df_temp = df.copy()
        df_temp = self.sum_features(df_temp)
        df_temp = self.set_date_as_index(df_temp)
        df_temp = self.create_features(df_temp)
        df_temp = self.create_timeseries_features(df_temp)
        df_temp = self.set_index_as_index(df_temp)
        return df_temp
    
    def make_full_sample(self):
        """
        Create full sample dataset, including all floors year 2018 and 2019.
        """
        # create a list storing all dataframes
        datasets_list = [self.make_data_sample(floor=i) for i in range(1, 8)]
    ...
    def make_predict_score(self, y_test, prediction, print_result=False):
        """
    ...
    def insert_score(self, score_df, score_list_add: list, causal: str):
        """
    ...
    def imputer_iterative(self, df, zone):
        ...
        for e in zone:
            ...
            df_temp = self.form_dataframe(data_array=array_temp, old_dataframe=df[zone[e]])
        ...
        df_sample = self.process_data(df_sample)
        return df_sample

    def imputer_knn(self, df, zone):
        imputer = KNNImputer(n_neighbors=5, weights='uniform') # default parameters

        data_zone_list = []
        for e in zone:
            df_temp1 = self.set_date_as_index(df[zone[e]])
            df_temp2 = self.create_timeseries_features(df_temp1)
            df_temp2 = self.set_index_as_index(df_temp2)
            
            array_temp = imputer.fit_transform(df_temp2.drop(columns=['Date']))
            df_temp = self.form_dataframe(data_array=array_temp, old_dataframe=df_temp2)
    ...
    def imputer_meangroupby(self, df):
        df_sample = self.set_date_as_index(df)
        df_sample = self.create_timeseries_features(df_sample)
        ...
        df_sample = self.sum_features(df_sample)
        df_sample = self.create_features(df_sample)
        df_sample = self.set_index_as_index(df_sample)
        return df_sample